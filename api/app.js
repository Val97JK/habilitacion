...
app.get("/get-user-id", async (request,response) =>{
    ...
    const [rows, fields] = await connection.execute(`SELECT * FROM users where id_user='${id_user}'`);
    response.json(rows[0])
})
...
// Finalizan end point Valentina

//Estos son endpoint de Camila
...
//  require('./rutas/react.rutas')(app);

//  app.listen(puerto, ()=> {
//      console.log(`servicio iniciado a traves de la url http://localhost:${puerto}`)
// })